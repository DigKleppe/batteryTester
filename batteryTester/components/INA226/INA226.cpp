...
bool
INA226::waitConversionReady (uint32_t timeout)
{
...
bool
INA226::setBusVoltageConversionTime (uint8_t bvct)
{
  if (bvct > 7)
    return false;
  uint16_t mask = _readRegister (INA226_CONFIGURATION);
...
bool
INA226::setShuntVoltageConversionTime (uint8_t svct)
{
  if (svct > 7)
    return false;
  uint16_t mask = _readRegister (INA226_CONFIGURATION);
...
//
int
INA226::setMaxCurrentShunt (float maxCurrent, float shunt, bool normalize)
{
  ...
  if (normalize)
    {
      /*
         ...
         (due to unusual low resistor values in relation to maxCurrent) determines
         currentLSB we have to take the upper value for currentLSB

         (adjusted in 0.6.0)
         calculation of currentLSB based on shunt resistor and calibration
         register limits (2 bytes) cal = 0.00512 / ( shunt * currentLSB ) cal(max)
         = 2^15-1 currentLSB(min) = 0.00512 / ( shunt * cal(max) ) currentLSB(min)
         ~= 0.00512 / ( shunt * 2^15 ) currentLSB(min) ~= 2^9 * 1e-5 / ( shunt *
         2^15 ) currentLSB(min) ~= 1e-5 / 2^6 / shunt currentLSB(min) ~= 1.5625e-7
         / shunt
      */
      ...
      currentLSB_uA++; //  ceil() would be more precise, but uses 176 bytes of
                       //  flash.
...
//
bool
INA226::setAlertRegister (uint16_t mask)
{
...
bool
INA226::setAlertLimit (uint16_t limit)
{
...
//
uint16_t
INA226::_readRegister (uint8_t reg)
{
  _wire->beginTransmission (_address);
  _wire->write (reg);
...
uint16_t
INA226::_writeRegister (uint8_t reg, uint16_t value)
{
  _wire->beginTransmission (_address);
  _wire->write (reg);
  _wire->write (value >> 8);
