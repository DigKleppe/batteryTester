...
// History
//
// 2020-11-14  bperrybap - fixed timing issue in begin() on VERY fast processors
// like ESP using pinIO 2019.08.11  bperrybap - updates for reinitalization
// using begin() & init() and use of "new" 2019.05.30  bperrybap - updates to
// support use of "new" for lcd objects 2019.02.03  bperrybap -
// fatalError(errcode), accept negative errcode values 2017.12.23  bperrybap -
// added LCD API 1.0 init() function 2017.05.11  bperrybap - setCursor() wraps
// when auto linewrap enabled
...
// Unfortunately, it cannot be assumed that the hd44780 object is zereod out
// since users can create the lcd object using "new". Because of this, the
// constructor code must initalize certain variables
//
...
hd44780::hd44780(uint8_t cols, uint8_t rows, uint32_t chExecTimeUs,
                 uint32_t insExecTimeus)
    : _cols(cols), _rows(rows), _chExecTime(chExecTimeUs),
...
int hd44780::begin(uint8_t cols, uint8_t rows, uint8_t dotsize) {
   ...
   *	OR
   * if the user is changing the number of columns on the display since a
  ...
  if ((!_rowOffsets[0] && !_rowOffsets[1] && !_rowOffsets[2] &&
       !_rowOffsets[3]) ||
      (_cols && (_cols != cols))) {
   ...
   *
   * 2) Set the upper 4 bits (d4-d7) of Functionset: FUNCTION_SET|8BIT = 0x3
   *		LCD pulls in nibble as lower 4 bits to complete 8 bits of
   *command LCD interprets command 0x33. the LCD sees a Goto 8bit mode command
   *and changes to 8 bit mode.
   *
   * 3) Set the upper 4 bits (d4-d7) of Functionset: FUNCTION_SET|8BIT = 0x3
   ...
   * 1) Set the upper 4 bits (d4-d7) of Functionset: FUNCTION_SET|8BIT = 0x3
   *		LCD pulls in nibble as lower 4 bits to complete 8 bits of
   *command LCD interprets command with upper nibble (unknown) and lower nibble
   *0x3 There is no telling what this command does as the upper nibble is
   *		unknown. And THAT is why the first delay in the sequence must be
   *so long. It isn't that the the first "function set" takes that long it is
   *because there is no telling what command might now be executing and BUSY
   *cannot be used yet. Therefore, the host must blind wait the worst case
   *amount of time. A 4.1ms delay appears to be 2.7x the worst case instruction
   *		execution time (clear & home are 1.52ms) at a 270kHz LCD clock
   *rate. (4.1ms would accomodate a chip clocked at 100kHz instead of 270 kHz)
   *
   * 2) Set the upper 4 bits (d4-d7) of Functionset: FUNCTION_SET|8BIT = 0x3
   *		If LCD happened to go into 8BIT mode in step 1
   *			LCD interprets this command as 0x30
   *				since d0-d3 lower 4 bits will be read as zeros in 8
   *bit mode This is essentially a NOP as the display is already in 8 bit mode
   *
   ...
   *
   *		If LCD still in 4 bit mode
   *			LCD pulls in nibble as lower 4 bits to complete 8 bits
   *of cmd LCD interprets command 0x33. the LCD sees a Goto 8bit mode command
   *and changes to 8 bit mode.
   *
   *
   * =======================================================================
   ...
   * like yield() to potentially allow other code to execute during this time
   *period.
   *
...
int hd44780::setCursor(uint8_t col, uint8_t row) {
  ...
  // it is going to adjust them to maintain compability with existing
  // LiquidCrystal library and to allow a back door way to set the ddram address
  // - see below
  ...
  if (!(_displaymode & HD44780_ENTRYLEFT2RIGHT))
    return (
        command(HD44780_SETDDRAMADDR | (_cols - col - 1 + _rowOffsets[row])));
  else
#endif
...
// These API functions scroll/shift the display contents without changing the
// RAM
...
// turn off backlight
int hd44780::noBacklight(void) { return (iosetBacklight(0)); }
...
//	failure: neagative value (error or read/status not supported by i/o
// subclass
int hd44780::status() {
...
int hd44780::blinkLED(int blinks) {
  ...
  if (blinks) {
  } // "nop" if to eliminate warning, will be optimized out
