...
public:
  ...
  hd44780(uint8_t cols, uint8_t rows, uint32_t chExecTime,
          uint32_t insExecTime);
  ...
  size_t __attribute__((error("println() is not supported")))
  ...
  inline void __attribute__((deprecated("Use setExecTimes() instead")))
  setDelay(uint32_t CmdDelay, uint32_t CharDelay) {
...
#if (__GNUC__ >= 4) && (__GNUC_MINOR__ >= 5)
  inline int __attribute__((deprecated("Use cursor() instead"))) cursor_on() {
    return (cursor());
  }
  inline int __attribute__((deprecated("Use noCursor() instead")))
  cursor_off() {
    return (noCursor());
  }
  inline int __attribute__((deprecated("Use blink() instead"))) blink_on() {
    return (blink());
  }
  inline int __attribute__((deprecated("Use noBlink() instead"))) blink_off() {
    return (noBlink());
  }
#else
  ...
  // ==============================
  inline int setBacklight(uint8_t dimvalue) {
    return (iosetBacklight(dimvalue));
  }
  inline int setContrast(uint8_t contvalue) {
    return (iosetContrast(contvalue));
  }
  ...
  inline int createChar(uint8_t charval,
                        char charmap[]) // does not assume PROGMEM
  {
    return (createChar(charval, (uint8_t *)charmap));
  }

  // on processors that don't use/need PROGMEM it can be left off the
  // declaration
  int createChar(uint8_t charval, const uint8_t charmap[]); // assumes PROGMEM

  // this function is for compatibilty with other libraries like fm's
  // newliquidCrystal
  inline int createChar(uint8_t charval,
                        const char charmap[]) // assumes PROGMEM
  {
...
protected:
  ...
  inline void waitReady(int32_t offsetUs) {
    _waitReady((_startTime + offsetUs), _execTime);
  }
...
private:
  ...
  virtual int ioread(hd44780::iotype type) {
    if (type)
      return (RV_ENOTSUP);
    else
      return (RV_ENOTSUP);
  ...
  virtual int iosetBacklight(uint8_t dimvalue) {
    if (dimvalue)
      return (RV_ENOTSUP);
    else
      return (RV_ENOTSUP);
  } // optional
  virtual int iosetContrast(uint8_t contvalue) {
    if (contvalue)
      return (RV_ENOTSUP);
    else
      return (RV_ENOTSUP);
  ...
  inline void markStart(uint32_t exectime) {
    _startTime = (uint32_t)micros();
    _execTime = exectime;
  }
  uint32_t _chExecTime; // time in Us of execution time for clear/home
  uint32_t
      _insExecTime;    // time in Us of execution time for instructions or data
  uint32_t _startTime; // 'start' time of last thing sent to LCD (cmd or data)
  uint32_t
      _execTime; // execution time in Us of last thing sent to LCD (cmd or data)

  // internal API function to send only upper 4 bits of byte on LCD DB4 to DB7
  // pins
