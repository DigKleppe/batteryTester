...
private:
  ...
  static const uint8_t CUU_RS =
      (1 << 1); // RS bit. set for data, clear for command
  static const uint8_t CUU_RW =
      (1 << 2); // RW bit. set for read, clear for write
  ...
  int iosetBacklight(uint8_t dimvalue) {
    ...
    if (dimvalue) {
      // Data which follows the FunctionSet command is considered as brightness
      // data.
...
#if defined(SPI_HAS_TRANSACTION)
    if ((_data == MOSI) && (_clk == SCK)) {
      // NOTE: App note says min 500ns clock cycle, so 2Mhz is max, 4Mhz seems
      // to work.
