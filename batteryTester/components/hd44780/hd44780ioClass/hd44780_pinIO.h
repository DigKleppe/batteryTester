...
//
//
// 2020.08.01  bperrybap - removed calls to analogWrite() on ESP32 core (not
// supported) 2019.08.11  bperrybap - fixed bug introduced by broken backlight
// check tweak 2016.12.26  bperrybap - tweak to broken backlight check code
...
#define HIGHZ                                                                  \
  0xfe // value is not critical but it cannot be the same as LOW or HIGH, or
       // 0xff
...
public:
  ...
  hd44780_pinIO(uint8_t rs, uint8_t en, uint8_t d4, uint8_t d5, uint8_t d6,
                uint8_t d7) {
    _rs = rs;
    _en = en;
    _rw = 0xff; // not used
  ...
  hd44780_pinIO(uint8_t rs, uint8_t en, uint8_t d4, uint8_t d5, uint8_t d6,
                uint8_t d7, uint8_t bl, uint8_t blLevel) {
    _rs = rs;
    _en = en;
    _rw = 0xff; // not used
    _d4 = d4;
  ...
  hd44780_pinIO(uint8_t rs, uint8_t rw, uint8_t en, uint8_t d4, uint8_t d5,
  ...
  hd44780_pinIO(uint8_t rs, uint8_t rw, uint8_t en, uint8_t d4, uint8_t d5,
                uint8_t d6, uint8_t d7, uint8_t bl, uint8_t blLevel) {
    _rs = rs;
    _en = en;
    _rw = rw;
    _d4 = d4;
...
private:
  ...
  int iosetBacklight(uint8_t dimvalue) {
    ...
    // Check for HIGHZ active level which is used on LCDkeypads with broken
    // backlight circuits In HIGHZ active level, the output is is never driven
    // high, instead the pin is put into input mode to turn on the backlight. To
    // turn off the backlight, the pin is flipped to output mode. because the
    // pin is never set to HIGH, when the pin is set to output mode it will be
    // LOW.
