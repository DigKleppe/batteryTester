...
public:
  ...
  // Note: This doesn't work for Leonardo since SS is not available and only
  // drives an LED
...
// This requires that <SPI.h> be included prior to this header
#if (defined(SPI_HAS_TRANSACTION) && !defined(ARDUINO_ARCH_ESP32)) || (defined(ARDUINO_ARCH_ESP32) && defined(_SPI_H_INCLUDED) && defined(SPI_HAS_TRANSACTION))
  ...
  //
  int
  iowrite (hd44780::iotype type, uint8_t value)
  {
    ...
    // upper 32 bytes are for commands so drop chars that are in the 0xE0 to
    // 0xFF range
    if (value >= 0xE0)
      {
        return (0);
  ...
  int
  iosetBacklight (uint8_t dimvalue)
  {
    if (dimvalue)
      {
  ...
  devicewrite (uint8_t value)
  {
    ...
    // ESP32 core always defines SPI_HAS_TRANSACTION even when SPI.h is not
    // included
  ...
  //
  void
  myshiftOut (uint8_t dataPin, uint8_t clockPin, uint8_t bitOrder, uint8_t val)
  {
    uint8_t i;
...
// and allow a bit of time for slew rate rise when using 3v processor with 5v
// logic
#if (F_CPU > 40000000)
