...
// hd44780_I2Cexp lcd(0x20, I2Cexp_MCP23008,1,2,3,4,5,6,7,HIGH);   // no rw
// support hd44780_I2Cexp lcd(0x27, I2Cexp_PCF8574, 0,1,2,4,5,6,7,3,HIGH); //
// with rw support
//
// hd44780_I2Cexp lcd(addr, canned-entry);
// hd44780_I2Cexp lcd(0x20, I2Cexp_BOARD_ADAFRUIT292); // specific backpack at
// 0x20 hd44780_I2Cexp lcd(0x38, I2Cexp_BOARD_SYDZ);        // specific backpack
// at 0x38
//
...
// 2016.12.25  bperrybap - new constructor for canned entry with no address for
// auto locate 2016.10.29  bperrybap - added sunrom canned entry
...
// boards that have control of the LCD r/w line will be able to do reads from
// lcd.
//
// The underlying hd4480_I2Cexp constructors support
// with or without r/w control, and with and without backlight control.
// - If r/w control is not desired, simply leave off the r/w pin from the
// constructor.
// - If backlight control is not desired/supported, simply leave off backlight
// pin and active level
//
...
//
//									expType,
// rs[,rw],en,d4,d5,d6,d7[,bl, blLevel]
#define I2Cexp_BOARD_LCDXIO                                                    \
  I2Cexp_PCF8574, 4, 5, 6, 0, 1, 2,                                            \
      3 // ElectroFun default (no backlight control)
#define I2Cexp_BOARD_LCDXIOnBL                                                 \
  I2Cexp_PCF8574, 4, 5, 6, 0, 1, 2, 3, 7,                                      \
      LOW // Electrofun & PNP transistor for BL

#define I2Cexp_BOARD_MJKDZ                                                     \
  I2Cexp_PCF8574, 6, 5, 4, 0, 1, 2, 3, 7, LOW // mjkdz backpack
#define I2Cexp_BOARD_GYI2CLCD                                                  \
  I2Cexp_PCF8574, 6, 5, 4, 0, 1, 2, 3, 7, LOW // GY-I2CLCD backpack

#define I2Cexp_BOARD_LCM1602                                                   \
  I2Cexp_PCF8574, 0, 1, 2, 4, 5, 6, 7, 3, LOW // Robot Arduino LCM1602 backpack
                                              // (jumper forces backlight on)

// these boards are all the same
// and match/work with the hardcoded Arduino LiquidCrystal_I2C class
#define I2Cexp_BOARD_YWROBOT                                                   \
  I2Cexp_PCF8574, 0, 1, 2, 4, 5, 6, 7, 3,                                      \
      HIGH // YwRobot/DFRobot/SainSmart/funduino backpack
#define I2Cexp_BOARD_DFROBOT                                                   \
  I2Cexp_PCF8574, 0, 1, 2, 4, 5, 6, 7, 3,                                      \
      HIGH // YwRobot/DFRobot/SainSmart/funduino backpack
#define I2Cexp_BOARD_SAINSMART                                                 \
  I2Cexp_PCF8574, 0, 1, 2, 4, 5, 6, 7, 3,                                      \
      HIGH // YwRobot/DFRobot/SainSmart/funduino backpack
#define I2Cexp_BOARD_FUNDUINO                                                  \
  I2Cexp_PCF8574, 0, 1, 2, 4, 5, 6, 7, 3,                                      \
      HIGH // YwRobot/DFRobot/SainSmart/funduino backpack
#define I2Cexp_BOARD_SUNROM                                                    \
  I2Cexp_PCF8574, 0, 1, 2, 4, 5, 6, 7, 3,                                      \
      HIGH // YwRobot/DFRobot/SainSmart/funduino backpack
           // http://www.sunrom.com/p/i2c-lcd-backpack-pcf8574
// not recommended
#define I2Cexp_BOARD_SYDZ                                                      \
  I2Cexp_PCF8574, 0, 1, 2, 4, 5, 6, 7, 3,                                      \
      HIGH // YwRobot/DFRobot/SainSmart/funduino backpack
...
#define I2Cexp_BOARD_SY1622                                                    \
  I2Cexp_PCF8574, 0, 1, 2, 4, 5, 6, 7, 3,                                      \
      HIGH // This board uses a FET for backlight control
           // This breaks the autodetection.

// MCP23008 based boards
// Currently r/w control is disabled since most boards either can't do it, or
// have it disabled.
#define I2Cexp_BOARD_ADAFRUIT292                                               \
  I2Cexp_MCP23008, 1, 2, 3, 4, 5, 6, 7,                                        \
      HIGH // Adafruit #292 i2c/SPI backpack in i2c mode (lcd RW grounded)
           // GP0 not connected to r/w so no ability to do LCD reads

#define I2Cexp_BOARD_WIDEHK                                                    \
  I2Cexp_MCP23008, 4, 7, 0, 1, 2, 3, 6,                                        \
      HIGH // WIDE.HK mini backpack (lcd r/w hooked to GP5)

#define I2Cexp_BOARD_LCDPLUG                                                   \
  I2Cexp_MCP23008, 4, 6, 0, 1, 2, 3, 7,                                        \
      HIGH // JeeLabs LCDPLUG (NOTE: NEVER use the SW jumper)
           // GP5 is hooked to s/w JP1 jumper, LCD RW is hardwired to gnd
           // So no ability to do LCD reads.

#define I2Cexp_BOARD_EFREAK                                                    \
  I2Cexp_MCP23008, 7, 6, 5, 4, 3, 2, 1,                                        \
      HIGH // elecfreaks backpack. (lcd RW grounded)
           // very similar design to Adafruit board but uses different pin
           // mapping
...
#define I2Cexp_BOARD_MLTBLUE                                                   \
  I2Cexp_MCP23008, 1, 3, 4, 5, 6, 7, 0,                                        \
      HIGH // i2c LCD MLT group "Blue Board" backpack
...
// FIXME these can't go in the class unless they are referenced using the
// classname
...
public:
  ...
  hd44780_I2Cexp(I2CexpType type, uint8_t rs, uint8_t rw, uint8_t en,
                 ...
                 uint8_t blLevel) {
    config(0, type, rs, rw, en, d4, d5, d6, d7, bl,
           blLevel); // auto locate i2c address
  }

  // Auto locate but with explicit config no r/w control with backlight control
  hd44780_I2Cexp(I2CexpType type, uint8_t rs, uint8_t en, uint8_t d4,
                 uint8_t d5, uint8_t d6, uint8_t d7, uint8_t bl,
                 uint8_t blLevel) {
    config(0, type, rs, 0xff, en, d4, d5, d6, d7, bl,
           blLevel); // auto locate i2c address
  }

  // Auto locate but with explicit config no r/w control with no backlight
  // control
  hd44780_I2Cexp(I2CexpType type, uint8_t rs, uint8_t en, uint8_t d4,
  ...
  hd44780_I2Cexp(uint8_t i2c_addr, I2CexpType type, uint8_t rs, uint8_t rw,
                 uint8_t en, uint8_t d4, uint8_t d5, uint8_t d6, uint8_t d7) {
...
private:
  ...
  int ioinit() {
    int status = 0;
    // auto instance tracts inst number when creating multiple auto locate
    // objects this is static since it is for the entire class not per object.
  ...
  returnStatus:
    ...
    // try to put gpio port back to all outputs state with WR signal low for
    // writes
    Wire.beginTransmission(_addr);
  ...
  void config(uint8_t i2c_addr, I2CexpType i2c_type, uint8_t rs, uint8_t rw,
              uint8_t en, uint8_t d4, uint8_t d5, uint8_t d6, uint8_t d7,
   ...
   * active level because that backpack uses a pullup on the backlight
   *transistor base. SYDZ backpacks will have to be manually configured.
   *
   ...
   * When there is a pullup on the NPN base, it will still be pulled down low
   *enough to read as a low as long as the emitter is wired directly to ground.
   *
   * NOTE: While normally wiring an i/o pin directly to a NPN base when the
   *emitter is connected directly to ground would be bad when the i/o pin is
   *driven high (it shorts), It is not an issue with the PCF8574 since the
   *PCF8574 does not drive i/o pins. The PCF8574 will enable a pullup when the
   *pin is set to "high". The path through the transistor base, out the emitter
   *to ground will pull the signal down to read as a low.
   *
   ...
   * this determines that the upper 4 bits are lcd data lines and the lower 3
   *bits are the lcd control lines with bit 2 being the E signal. this means
   *rs=0,rw=1,en=2,d4=4,d5=5,d6=6,d7=7,bl=3
   *
   * If bits 4-6 are all 1s and bits 0-3 are 1s when bit 4 is off
   * this determines that the lower 4 bits are lcd data lines and the upper 3
   *bits are the lcd control lins with bit 4 being the E signal. this means
   *rs=6,rw=5,en=4,d4=0,d5=1,d6=2,d7=3,bl=7
   *
   * If bits 4-6 are all 1s and bits 0-3 are 1s when bit 6 is off
   * this determines that the lower 4 bits are lcd data lines and the upper 3
   *bits are the lcd control lins with bit 6 being the E signal. this means
   ...
   * a NPN transistor base *AND* the BL anode is wired to the emitter instead
   *wiring the BL cathode to the colector. With an FET there is no load betweent
   *the 8574 bl pin and the FET gate, so there is no way to distinguish this
   *from a PNP emitter pulling up a base pin. When using a NPN transistor with a
   *pullup, the pullup on the base will not allow the base to be pulled in the
   *direction of the emitter if the BL anode is wired to the emitter.
   *
   *
   ...
   * of the collector.
   * Because of this, the code will incorrecly pick active low backlight
   *control. Modifying the h/w to disable the pullup can be done, but is
   *difficult due to the way R1 resistors are used and the PCB layout. It
   *requires cuting a trace and dead bugging a new resistor to  connect to the
  ...
  int autocfg8574() {
    ...
    // If lower 3 bits are high and all 4 upper bits are high after clearing bit
    // 2 then lower bits are control bits and upper bits are data bits and bit2
    // was en.
